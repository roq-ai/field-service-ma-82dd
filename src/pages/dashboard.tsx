...
import { getPartsOrders } from 'apiSdk/parts-orders';
import { PartsOrderInterface } from 'interfaces/parts-order';

import { ActivityListPage as ShowCaseList } from 'pages/activities';
...
function DashboardPage() {
  ...
  const partsOrderStat = useSWR<PaginatedInterface<PartsOrderInterface>>(
    () => `/parts-orders`,
    () => getPartsOrders({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Parts Orders',
      result: partsOrderStat?.data?.totalCount,
      isLoading: partsOrderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`activities`} passHref legacyBehavior>
                  <Button
